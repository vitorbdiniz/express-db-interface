...
def execute_operations(insert, data=None, trail=None, update=False, order=1, productId=None, CourseTrailOrder=1,
                       env='hml'):
    """
    ...
    if insert == 'trail':
        ...
        result = writer.insert_course('Course',
                                      ...
                                      objective='Os fundos imobiliários são uma ótima alternativa para quem busca se expor a uma classe de ativos relacionada ao mercado imobiliário. O que são os fundos imobiliários e como analisá-los, além das questões tributárias e de como montar uma carteira, são alguns conhecimentos que você adquire com este curso.',
                                      certification=None,
                                      ...
                                      timeAvailable=None,
                                      topics=None,
                                      backgroundUrl='https://tc.com.br/wp-content/uploads/2021/12/Fiis-Desktop_357x272.jpg',
                                      sequential=1,
                                      ...
                                      isOpenCourse=0,
                                      imgMobileUrl='https://tc.com.br/wp-content/uploads/2021/12/Fiis-Mobile_375x667.jpg',
                                      imgBannerUrl='https://tc.com.br/wp-content/uploads/2021/12/Fiis-Desktop_1920x1080.jpg',
                                      courseSlug=None,
...
def main():
    ...
    if not insert_all:
        item = ['module', 'lecture', 'article', 'refference', 'file']
    else:
        item = []
    ...
    for insert in get_inserts(insert_all, insert_in_trail, item):
        if name == 'experience.csv':
            for turma in turmas:
                ...
                result = execute_operations(insert, data=data, trail=trail, update=True, order=10, productId=productId,
                                            CourseTrailOrder=CourseTrailOrder, env=env)
        else:
            # exit(data)
            result = execute_operations(insert, data=data, trail=trail, update=True, order=10, productId=productId,
                                        CourseTrailOrder=CourseTrailOrder, env=env)
